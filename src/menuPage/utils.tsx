import { MenuItem, Data ,MenuTypeEnum} from "./type";
...
//三、聚焦到整个菜单，删除菜单项处理
export function removeOutput(befVal: MenuItem[], nowVal: MenuItem[], output, slot) {
...
//四、聚焦到整个菜单，增加菜单项处理
export function addOutput(befVal: MenuItem[], nowVal: MenuItem[], output, slot) {
  const schema = {
    type: 'any'
  };
  //增加子菜单点击事件
  for (let i = 0; i < [...nowVal].length; i++) {
    ...
    slot.add([...nowVal][i]._key, `${[...nowVal][i].title}`)
  }
...
//十三、顶部菜单对应项取值
export const getTopMenuItem = ({ data, focusArea }: EditorResult<Data>, path?: keyof MenuItem) => {
  if (!focusArea) return;
  const key = focusArea.dataset['topMenuItem'];
  const item = findMenuItem(data.topDataSource, key, true);
  if (!path) {
    return item;
  }
  return item?.[path];
...
//十四、顶部菜单对应项赋值
export const setTopMenuItem = <T extends keyof MenuItem, P extends MenuItem[T]>(
  { data, focusArea }: EditorResult<Data>,
  path: T,
  value: P
) => {
  ...
  data.topDataSource = [...data.topDataSource];
};